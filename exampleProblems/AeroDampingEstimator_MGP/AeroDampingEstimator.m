function [problem,guess] = AeroDampingEstimator
...
auxData.HF.RRH = auxData.HF.RRH - interp1(trackData.dsResult.dsState.time, dsResult.dsState.chassis.sprung.ride_height_R, auxData.HF.tLap, 'linear', 'extrap');
...
auxData.LF.kHeave = interp1(trackData.dsResult.dsState.time, dsResult.dsControl.dsLapReplayGGVS_out.data.car.KCarHeave, auxData.HF.tLap, 'linear', 'extrap');
auxData.LF.kHeaveInterval = auxData.LF.kHeave(auxData.HF.interval);

%% Plant model name, used for Adigator
InternalDynamics=@Dynamics_AeroDamping_Internal;
...
% u2 - cs;
% u3 - ca;
problem.inputs.ul=[0, 0 -inf];
problem.inputs.uu=[inf inf inf];

% Bounds on the first control action
problem.inputs.u0l=[0 0 -inf];
...
problem.inputs.uConstraintTol=[10, 10, 10];

% Guess the input sequences with [u0 uf]
guess.inputs(:, 1) = 300e3*ones(length(auxData.HF.intervalTime), 1);
guess.inputs(:, 2) = 20e3*ones(length(auxData.HF.intervalTime), 1);
