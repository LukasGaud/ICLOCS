function [problem,guess] = AeroDampingEstimator_Param_PSD(auxData)
...
InternalDynamics=@Dynamics_Internal_Param_PSD_for_ipopt;
...
problem.parameters.pl=[0, 0, -1e7, -1e7, 0];
problem.parameters.pu=[10*auxData.P.kt, 10*auxData.P.ct, 1e7, 1e7, 10];
guess.parameters=[auxData.P.kt, auxData.P.ct, 0, 0, 0.001];
...
guess.inputs = zeros(length(auxData.HF.intervalTime),1);% %% Set points
...
[track.x1_psd, track.y1_psd] = fnPSD(t, sampledTrackX1, 'HighPassFreq', 3);
[track.x2_psd, track.y2_psd] = fnPSD(t, sampledTrackX2, 'HighPassFreq', 3);
[track.x3_psd, track.y3_psd] = fnPSD(t, sampledTrackX5, 'HighPassFreq', 3);
