...
for iData = 1:size(dataList,1)
    ...
    x0 = [solutionConst{iData}.p(1), solutionConst{iData}.p(2),solutionConst{iData}.p(3), solutionConst{iData}.p(4), 0.01];
    lb = [-inf, -inf, 0];
    ub = [inf, inf, 1];
...
%     p = fmincon(@(x)fDampingEst_PSD(x, problem{iData}.data.auxData, solution{iData}.p, solution{iData}.U(:,3), roadVec), x0, [], [], [], [], lb, ub, [], opt);
     p = fminsearch(@(x)fDampingEst_PSD(x, problem{iData}.data.auxData, solution{iData}.p, solution{iData}.U(:,3), roadVec), x0, opt);
%     p = fminunc(@(x)fDampingEst_PSD(x, problem{iData}.data.auxData, solution{iData}.p, solution{iData}.U(:,3), roadVec), x0, opt);
     fDampingCheck_PSD(p, problem{iData}.data.auxData, solution{iData}.p, solution{iData}.U(:,3), roadVec);
end
fPlotFigures_sLap
%% Eigenvalue plotting
