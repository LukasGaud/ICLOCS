function [problem,guess] = AeroDampingEstimator_Param_sLap(auxData, zrDot)
...
InternalDynamics=@Dynamics_Bouncing_Internal_Param;
...
% p(1) = kt, p(2) = ct, p(3) = ka, p(4) = ca
problem.parameters.pl=[auxData.P.kt-0.5*auxData.P.kt, 0, -1e7, -1e7];
problem.parameters.pu=[auxData.P.kt+auxData.P.kt, 2*auxData.P.ct, 1e7, 1e7];
guess.parameters=[auxData.P.kt, auxData.P.ct, 0, 0];
...
problem.inputs.uConstraintTol=[0];

% Guess the input sequences with [u0 uf]
guess.inputs = zeros(length(auxData.HF.intervalTime),1);
%% Set points
...
% Add zrDot to the auxData 
auxData.Inputs.zrDot = zrDot;
...
dataU = vdat.auxData.Inputs.zrDot;
...
sampledU = interp1(dataTime, dataU, t, 'linear', 'extrap');


e1 = x(:,1) - sampledTrackX1;
e2 = x(:,2) - sampledTrackX2 - sampledU;
