function [dx,g_neq] = Dynamics_AeroDamping_Internal_with_RoadEstimation(x,u,p,t,vdat)
...
% Getting ks and cs
ksDef = interp1(vdat.auxData.HF.intervalTime, vdat.auxData.LF.kHeaveInterval, t, 'linear', 'extrap');
csDef = interp1(vdat.auxData.P.vDamVec, vdat.auxData.P.csVec, x2-x4, 'linear', 'extrap');

dx(:,1) = x2 - zr;

dx(:,2) = (-ks./ms - ka./ms).*x1 + (-cs./ms - ca./ms).*x2 + (ks./ms).*x3 + (cs./ms).*x4 + Fs./ms + ca./ms.*zr;
...
c1 = abs(cs-csDef)./csDef + 0.05*ones(length(cs),1);
c2 = (-abs(cs-csDef)./csDef) + 0.05*ones(length(cs),1);
c3 = abs(ks-ksDef)./ksDef + 0.05*ones(length(cs),1);
c4 = (-abs(ks-ksDef)./ksDef) + 0.05*ones(length(cs),1);
g_neq = [c1,c2,c3,c4];
%------------- END OF CODE --------------