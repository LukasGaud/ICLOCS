function [problem,guess] = AeroDampingEstimator(bLS)
...
FDam = [flip(trackData.dsResult.dsControl.dsVd.car.rearSuspension.heaveDamper.extension(:,2)); trackData.dsResult.dsControl.dsVd.car.rearSuspension.heaveDamper.compression(:,2)];
vDam = [flip(trackData.dsResult.dsControl.dsVd.car.rearSuspension.heaveDamper.extension(:,1)); trackData.dsResult.dsControl.dsVd.car.rearSuspension.heaveDamper.compression(:,1)];
...
cutOffSpeed = 0.5;
pDam = polyfit(vDam(abs(vDam)<cutOffSpeed), FDam(abs(vDam)<cutOffSpeed), 4);
...
% kStiffness from SVA
xBumpVec = trackData.dsResult.dsControl.dsPolarisSVA_out.data.car.rearSuspension.left.wheelHub.xBump;
ksVec = trackData.dsResult.dsControl.dsPolarisSVA_out.data.car.rearSuspension.KWheelRateDynamicRL;

auxData.P.ksVec = ksVec*motionRatio^2;
...
auxData.HF.zRHeaveDot = [dzRHeave./dT; 0];
auxData.HF.zRHeaveDDot = [diff(auxData.HF.zRHeaveDot)./dT; 0];
...
auxData.HF.RRH = auxData.HF.RRH - interp1(trackData.dsResult.dsState.time, trackData.dsResult.dsState.chassis.sprung.ride_height_R, auxData.HF.tLap, 'linear', 'extrap');
dT = diff(auxData.HF.tLap);
auxData.HF.RRHDot = lowpass([diff(auxData.HF.RRH)./dT, 0], fPass, 1/deltaTime);
...
auxData.LF.kHeave = interp1(trackData.dsResult.dsState.time, trackData.dsResult.dsControl.dsLapReplayGGVS_out.data.car.KCarHeave, auxData.HF.tLap, 'linear', 'extrap');
auxData.LF.kHeaveInterval = auxData.LF.kHeave(auxData.HF.interval);
auxData.LF.RRH = interp1(trackData.dsResult.dsState.time, trackData.dsResult.dsState.chassis.sprung.ride_height_R, auxData.HF.tLap, 'linear', 'extrap');
auxData.LF.RRHInterval = auxData.LF.RRH(auxData.HF.interval);

%% Plant model name, used for Adigator
InternalDynamics=@Dynamics_AeroDamping_Internal_with_RoadEstimation;
...
% Settings file
problem.settings=@settings_AeroDamping;
...
problem.states.x0=[auxData.HF.RRHInterval(1)-auxData.HF.zRHeaveInterval(1), auxData.HF.RRHDotInterval(1), auxData.LF.zRearHubInterval(1)-auxData.HF.zRHeaveInterval(1), auxData.LF.zDotRearHubInterval(1)];

% Initial conditions for system. Bounds if x0 is free s.t. x0l=< x0 <=x0u
problem.states.x0l=[-0.5 -1, -0.5, -1];
problem.states.x0u=[0.5, 1, 0.5, 1];  
...
problem.states.xfl=[min(auxData.HF.RRHInterval-auxData.HF.zRHeaveInterval)-0.01, min(auxData.HF.RRHDotInterval)-0.01, -0.5, -1];
problem.states.xfu= [max(auxData.HF.RRHInterval-auxData.HF.zRHeaveInterval)+0.01, max(auxData.HF.RRHDotInterval)+0.01, 0.5, 1];  
...
problem.inputs.ul= [-5e6, -5e6, 0, 0, -10];
problem.inputs.uu= [5e6, 5e6, 1e7, 1e7, 10];

% problem.inputs.ul= -5e6;
% problem.inputs.uu= 5e6;

% Bounds on the first control action
problem.inputs.u0l= [-5e5, -5e6, 0, 0, -10];
problem.inputs.u0u= [5e5, 5e6, 1e6, 1e7, 10];
...
problem.inputs.uConstraintTol=[10, 10, 10, 10, 0.1];

% problem.inputs.uConstraintTol= 10;

% Guess ks and cs
ksDef = auxData.LF.kHeaveInterval;
...
guess.inputs(:,5) = auxData.HF.zRHeaveDotInterval;
%% Set points
...
e1=x(:,1)-(sampledTrackX1 - cumtrapz(t, u(:,5)));
