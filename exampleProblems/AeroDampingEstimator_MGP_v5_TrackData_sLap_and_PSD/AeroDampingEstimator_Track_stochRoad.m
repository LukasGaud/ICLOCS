function [problem,guess] = AeroDampingEstimator_Track_stochRoad(trackData, sessionDetails, sLapStart, sLapEnd)
...
nNan = isnan(ksVec);
pSpring = polyfit(xHeaveVec(~nNan), ksVec(~nNan), 5);
kFit = polyval(pSpring, xBumpVec);
% kFit = interp1(xHeaveVec, ksVec, xBumpVec, 'linear');
auxData.P.xBumpVec = xBumpVec;
auxData.P.ksVec = kFit;
auxData.P.driver = sessionDetails('Driver');
auxData.P.NLap = trackData.NLap;
auxData.P.sLapStart = sLapStart;
auxData.P.sLapEnd = sLapEnd;
...
xHubR0 = (trackData.zSetupWheelCenterRR0_PLR + trackData.zSetupWheelCenterRL0_PLR)/2/1000;
auxData.P.xHubR0 = mean(xHubR0(~isnan(xHubR0)));
...
auxData.HF.xSetup = auxData.P.xHubR0 - auxData.HF.RRHSetup;

auxData.HF.dRRH = auxData.HF.RRH - mean(auxData.HF.RRHSetup);
...
[~, iStart] = min(abs(sLapStart - auxData.HF.sLap));
[~, iEnd] = min(abs(sLapEnd - auxData.HF.sLap));
...
problem.inputs.ul= [-5e7, -5e7, -1.5e4];
problem.inputs.uu= [5e7, 5e7, -1e4];

% problem.inputs.ul= -5e6;
% problem.inputs.uu= 5e6;

% Bounds on the first control action
problem.inputs.u0l= [-5e5, -5e6, -1.5e4];
problem.inputs.u0u= [5e5, 5e6, -1e4];
...
guess.inputs(:, 3) = auxData.HF.FzRearInterval;
%% Set points
...
dataRoad = vdat.auxData.HF.roadVecInterval;
...
sampledRoad = interp1(dataTime, dataRoad, t, 'linear', 'extrap');
