function [problem,guess] = AeroDampingEstimator_Param_Only_PSD(auxData)
...
InternalDynamics=@Dynamics_Internal_stochRoad_Param_Only;
...
% p(1) = kt, p(2) = ct, p(4) = ka, p(5) = ca, p(3) = alpha
problem.parameters.pl=[auxData.P.kt-0.5*auxData.P.kt, 0, 0, -5e6, -5e6];
problem.parameters.pu=[3*auxData.P.kt, 2*auxData.P.ct, 1, 5e6, 5e6];
guess.parameters=[auxData.P.kt, auxData.P.ct, 0.001, 0, 0];
...
problem.inputs.ul= -1.5e4;
problem.inputs.uu= -1e4;

% Bounds on the first control action
problem.inputs.u0l= -1.5e4;
problem.inputs.u0u= -1e4;
...
guess.inputs(:, 1) = auxData.HF.FzRearInterval;
