function [problem,guess] = AeroDampingEstimator_Track_sLap(trackData, sessionDetails, sLapStart, sLapEnd)
...
nNan = isnan(ksVec);
pSpring = polyfit(xHeaveVec(~nNan), ksVec(~nNan), 7);
...
problem.parameters.pl=[auxData.P.kt-0.5*auxData.P.kt, 0];
problem.parameters.pu=[auxData.P.kt+auxData.P.kt, 2*auxData.P.ct];
guess.parameters=[auxData.P.kt, auxData.P.ct];
...
problem.inputs.ul= [-5e7, -5e7,-10];
problem.inputs.uu= [5e7, 5e7, 10];
