function [problem,guess] = AeroDampingEstimator_Track(bLS, trackData)
...
auxData.HF.RRHDot = [diff(auxData.HF.RRH)/deltaTime, 0];
auxData.HF.xBumpRDot = [diff(auxData.HF.xBumpR)/deltaTime, 0];

auxData.HF.xHubR = auxData.HF.RRH - auxData.HF.xBumpR;
auxData.HF.xHubRDot = [diff(auxData.HF.xHubR)/deltaTime, 0];
auxData.HF.xHubR_HP = highpass(auxData.HF.xHubR, fHP, 1/deltaTime);
...
auxData.HF.xHubRInterval = auxData.HF.xHubR(auxData.HF.interval);
auxData.HF.xHubRDotInterval = auxData.HF.xHubRDot(auxData.HF.interval);
...
InternalDynamics=@Dynamics_AeroDamping_Internal_with_RoadEstimation_Track;
...
zr0 = -0.02;
problem.states.x0=[auxData.HF.RRHInterval(1)-zr0, auxData.HF.RRHDotInterval(1), auxData.HF.xHubRInterval(1)-zr0, auxData.HF.xHubRDotInterval(1)];
...
x1Guess = auxData.HF.RRHInterval-zr0;
x2Guess = auxData.HF.RRHDotInterval;
x3Guess = auxData.HF.xHubRInterval-zr0;
...
problem.inputs.ul= [-5e6, -5e6, 0, 0, -1];
problem.inputs.uu= [5e6, 5e6, 1e7, 1e7, 1];

% problem.inputs.ul= -5e6;
% problem.inputs.uu= 5e6;

% Bounds on the first control action
problem.inputs.u0l= [-5e5, -5e6, 0, 0, -1];
problem.inputs.u0u= [5e5, 5e6, 1e6, 1e7, 1];
...
problem.inputs.uConstraintTol=[10, 10, 10, 10, 0.01];
...
guess.inputs(:,5) =  zr0*ones(length(auxData.HF.intervalTime), 1);
%% Set points
...
e1 = x(:,1)-(sampledTrackX1 - u(:,5));
e2 = x(:,2) - sampledTrackX2;
e3 = x(:,3) -(sampledTrackX3 - u(:,5));
