function [dx,g_neq] = Dynamics_AeroDamping_Internal_with_RoadEstimation_Track(x,u,p,t,vdat)
...
% Getting ks and cs
ksDef = interp1(vdat.auxData.P.xBumpVec, vdat.auxData.P.ksVec, x1-x3, 'linear', 'extrap');
csDef = interp1(vdat.auxData.P.vDamVec, vdat.auxData.P.csVec, x2-x4, 'linear', 'extrap');

dx(:,1) = x2 - zrDot;

dx(:,2) = (-ks./ms - ka./ms).*x1 + (-cs./ms - ca./ms).*x2 + (ks./ms).*x3 + (cs./ms).*x4 + Fs./ms + ca./ms.*zrDot;
% dx(:,2) = (-ks./ms).*x1 + (-cs./ms - ca./ms).*x2 + (ks./ms).*x3 + (cs./ms).*x4 + Fs./ms + ca./ms.*zr;
...
% dx(:,1) = x2;
% 
% dx(:,2) = (-ks./ms - ka./ms).*x1 + (-cs./ms - ca./ms).*x2 + (ks./ms).*x3 + (cs./ms).*x4 + Fs./ms + ca./ms.*zrDot + ka./ms.*zr;
% % dx(:,2) = (-ks./ms).*x1 + (-cs./ms - ca./ms).*x2 + (ks./ms).*x3 + (cs./ms).*x4 + Fs./ms + ca./ms.*zr;
% 
% dx(:,3) = x4;
% 
% dx(:,4) = (ks./mu).*x1 + (cs./mu).*x2 + (-ks./mu - kt./mu).*x3 + (-cs./mu - ct./mu).*x4 + (ct/mu)*zrDot + kt/mu.*zr;

c1 = abs(cs-csDef)./csDef + 0.1*ones(length(cs),1);
c2 = (-abs(cs-csDef)./csDef) + 0.1*ones(length(cs),1);
c3 = abs(ks-ksDef)./ksDef + 0.1*ones(length(cs),1);
c4 = (-abs(ks-ksDef)./ksDef) + 0.1*ones(length(cs),1);
g_neq = [c1,c2,c3,c4];
%------------- END OF CODE --------------